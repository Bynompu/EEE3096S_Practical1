...
volatile uint8_t current_pattern = 0; //set current pattern to the first pattern
volatile uint32_t delay_time = 2000; // Default delay time of 1 second
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim16); //starting and initializing the TIM16
  /* USER CODE END 2 */
  ...
  while (1)
  {
	...
	// Code to check pushbuttons to change timer delay
...
void TIM16_IRQHandler(void)
{
	...
	// print something
    current_pattern = (current_pattern + 1) % 9; // Cycle through the nine LED patterns
    set_led_pattern(led_patterns[current_pattern]);

}

// Code to set the LED patterns
...
// Code to update timer delay time
void update_delay_time(uint32_t delay) {
    htim16.Instance->ARR = (delay * 8) - 1; // Update the Auto Reload Register for new delay values
